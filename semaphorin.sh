...
_boot() {
    if [[ "$cpid" == "0x8003" || "$cpid" == "0x8000" || "$cpid" == "0x8010" || "$cpid" == "0x8011" || "$cpid" == "0x8015" ]]; then
        ...
        if [[ "$3" == "7."* ]]; then
            ...
            echo "xargs $boot_args rd=disk0s1s1" | "$bin"/pongoterm
        fi
    ...
    if [[ "$restore" == 1 ]]; then
        ...
        if [[ "$version" == "7."* || "$version" == "8."* || "$version" == "9."* || "$version" == "10.0"* || "$version" == "10.1"* || "$version" == "10.2"*  ]]; then
            ...
            if [[ "$version" == "10."* ]]; then
                ...
                "$bin"/sshpass -p "alpine" scp -o StrictHostKeyChecking=no -P 2222 "$dir"/jb/yalu102.app.tar.gz root@localhost:/mnt1/
                "$bin"/sshpass -p 'alpine' ssh -o StrictHostKeyChecking=no -p2222 root@localhost 'tar --preserve-permissions -xzvf /mnt1/yalu102.app.tar.gz -C /mnt1/Applications' 2> /dev/null
                "$bin"/sshpass -p 'alpine' ssh -o StrictHostKeyChecking=no -p2222 root@localhost 'rm -rf /mnt1/yalu102.app.tar.gz' 2> /dev/null
                if [[ ! "$deviceid" == "iPad"* ]]; then
                ...
                if [[ ! -e "$dir"/$deviceid/$cpid/$version/dyld_shared_cache_arm64.patched ]]; then
            ...
            if [[ "$version" == "10."* ]]; then
                ...
                "$bin"/sshpass -p 'alpine' ssh -o StrictHostKeyChecking=no -p2222 root@localhost "mv /mnt2/staged_system_apps/* /mnt1/Applications"
                "$bin"/sshpass -p "alpine" scp -o StrictHostKeyChecking=no -P 2222 "$dir"/jb/com.apple.Accessibility.Collection.plist root@localhost:/mnt2/mobile/Library/Preferences/com.apple.Accessibility.Collection.plist
                "$bin"/sshpass -p "alpine" scp -o StrictHostKeyChecking=no -P 2222 "$dir"/jb/com.apple.Accessibility.plist root@localhost:/mnt2/mobile/Library/Preferences/com.apple.Accessibility.plist
